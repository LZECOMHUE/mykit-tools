    ...
    <div className="space-y-4">
      ...
      {!image && (
        ...
        >
          ...
          <p className="font-medium text-text-primary mb-1">Drop a WebP here or click to upload</p>
          <p className="text-sm text-text-muted">WebP or any image format</p>
          <input ref={fileInputRef} type="file" accept="image/webp,image/*" onChange={handleUpload} className="hidden" />
        </div>
      )}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-text-secondary mt-0.5">
                  <span className="font-mono">{fileInfo?.width} x {fileInfo?.height}px</span> - lossless quality preserved
                </p>
              </div>
            </div>
          </Card>
          ...
          <Card>
            <div className="grid grid-cols-3 gap-3 text-center">
              <div>
                ...
                <p className="text-xs text-text-muted mt-0.5">WebP</p>
              </div>
            ...
            {sizeDiff !== null && (
              <p className="text-center text-xs text-text-muted mt-3">
                {sizeDiff > 0
                  ? `PNG is ${sizeDiff}% larger - this is normal, as PNG is a lossless format`
                  : `PNG is ${Math.abs(sizeDiff)}% smaller`}
              </p>
            )}
          </Card>
          ...
          <Card className="bg-blue-50 border-blue-200">
            <div className="flex items-start gap-2">
              ...
              <p className="text-xs text-text-secondary leading-relaxed">
                PNG is a universally supported lossless format. The file will be larger than the WebP original
                because PNG does not use lossy compression. PNG is ideal when you need maximum compatibility
                or plan to edit the image further.
              </p>
            </div>
          </Card>
