...
export default function AdjustImageContrast() {
  ...
  const drawCanvas = useCallback(() => {
    ...
    const filterValue = showOriginal ? 1 : (100 + contrast) / 100;
    ...
    offCtx.filter = `contrast(${filterValue})`;
    offCtx.drawImage(image, 0, 0);

    ctx.drawImage(offscreen, 0, 0);
  }, [image, contrast, showOriginal]);
  ...
  const handleDownload = () => {
    ...
    offCtx.filter = `contrast(${(100 + contrast) / 100})`;
    offCtx.drawImage(image, 0, 0);

    const link = document.createElement("a");
    link.download = `${fileName || "image"}-contrast.${ext}`;
