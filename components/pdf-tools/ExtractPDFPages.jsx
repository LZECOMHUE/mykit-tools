...
export default function ExtractPDFPages() {
  ...
  const handleExtractPages = async () => {
    ...
    try {
      ...
      for (const index of indicesToExtract) {
        const [copiedPage] = await newPdfDoc.copyPages(sourcePdfDoc, [index]);
    ...
    <div className="w-full max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-surface border border-border rounded-[var(--radius-card)] p-6">
        ...
        {selectedPages.size > 0 && (
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-[var(--radius-input)]">
            <p className="text-sm text-blue-700">
              <strong>{selectedPages.size} page{selectedPages.size !== 1 ? 's' : ''}</strong> will be extracted into a new PDF.
            </p>
          </div>
        )}
      </div>
