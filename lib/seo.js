...
// Country-prefixed tools whose pages we still want crawled but NOT indexed
// until proper SEO content is written for them. Removes thin pages from
// Google's quality-signal pool while keeping internal linking working.
const COUNTRY_PREFIX = /^(us|canada|australia|india)-/;

// Tier 3 tools (simple unit converters and one-formula calculators) are
// noindexed site-wide. Post-HCU, sister-tool clusters like kg-to-lbs and
// lbs-to-kg are read by Google as duplicate-content spam and drag the
// whole domain's quality score down. Keep them crawlable so internal
// links still flow PageRank to flagships, but pull them out of the index
// until each one earns its place individually.
function shouldNoindex(tool) {
  if (tool.tier === 3) return true;
  if (!COUNTRY_PREFIX.test(tool.slug)) return false;
  return getSEOContent(tool.slug) === null;
}
