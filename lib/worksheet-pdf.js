...
export function generateTrainingPlanPDF({
  ...
  plan.forEach((week, wIdx) => {
    ...
    days.forEach((day) => {
      ...
      doc.setFontSize(7);
      doc.setFont('helvetica', 'bold');
      const dayLabel = typeof day.day === 'number'
        ? ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][day.day] || ''
        : String(day.day || '');
      doc.text(dayLabel, MARGIN + 4, y + 2.5);

      doc.setTextColor(30, 30, 30);
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      const workout = String(day.workout || day.name || 'Rest');
      doc.text(workout, MARGIN + 22, y + 2.5);

      if (day.distance) {
        doc.setTextColor(accentColor);
        doc.setFont('helvetica', 'bold');
        doc.text(String(day.distance) + ' ' + units, PAGE_W - MARGIN - 4, y + 2.5, { align: 'right' });
      }
